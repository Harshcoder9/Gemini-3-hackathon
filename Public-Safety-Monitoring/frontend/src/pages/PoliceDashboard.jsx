...
import { acknowledgeAlert, fetchAlerts, fetchLocations, generateEmergencyResponse } from '../auth/api.js'
...
function EmergencyResponse({ response }) {
  ...
  const getUrgencyColor = (urgency) => {
    ...
    if (u === 'immediate') return 'bg-red-500/10 text-red-700 ring-red-500/20 dark:text-red-300'
    if (u === 'high') return 'bg-orange-500/10 text-orange-700 ring-orange-500/20 dark:text-orange-300'
    return 'bg-yellow-500/10 text-yellow-700 ring-yellow-500/20 dark:text-yellow-300'
  }
  
  return (
    <div className="mt-3 rounded-xl border border-indigo-200/70 bg-indigo-500/5 p-4 dark:border-indigo-500/20 dark:bg-indigo-500/10">
      <div className="flex items-start justify-between">
        <h4 className="text-sm font-bold text-indigo-900 dark:text-indigo-100">Emergency Response Recommendation</h4>
        <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${getUrgencyColor(response.urgency)}`}>
          {response.urgency || 'N/A'}
        </span>
      </div>
      
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        <div className="rounded-lg border border-indigo-200/50 bg-white/40 p-3 dark:border-indigo-500/10 dark:bg-white/5">
          <div className="text-xs font-semibold text-indigo-600 dark:text-indigo-300">Response Unit</div>
          <div className="mt-1 text-sm font-bold text-slate-900 dark:text-slate-100">{response.unit || 'Not specified'}</div>
        </div>
        
        <div className="rounded-lg border border-indigo-200/50 bg-white/40 p-3 dark:border-indigo-500/10 dark:bg-white/5">
          <div className="text-xs font-semibold text-indigo-600 dark:text-indigo-300">Primary Action</div>
          <div className="mt-1 text-sm font-bold text-slate-900 dark:text-slate-100">{response.action || 'Not specified'}</div>
          <div className="mt-1 text-sm font-bold text-slate-900 dark:text-slate-100">Emergency contact no:- 112, 102 & 108</div>
        </div>
      </div>
      
      {response.reasoning && response.reasoning.length > 0 && (
        <div className="mt-3 rounded-lg border border-indigo-200/50 bg-white/40 p-3 dark:border-indigo-500/10 dark:bg-white/5">
          <div className="text-xs font-semibold text-indigo-600 dark:text-indigo-300">Reasoning</div>
      ...
      {/* <div className="mt-2 text-xs text-indigo-600/70 dark:text-indigo-400/70">
        Generated by AI Decision Support System at {response.time || 'N/A'}
      </div> */}
    </div>
  )
}
...
function VoiceControls({ alert }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <select value={lang}  onChange={(e) => setLang(e.target.value)} className="rounded-xl border border-slate-200 bg-white/70 px-2 py-1.5 text-xs font-semibold text-slate-800 hover:bg-white dark:border-white/10 dark:bg-white/10 dark:text-slate-100 dark:hover:bg-white/15">
        <option value="en-US">🇬🇧 English</option>
        <option value="hi-IN">🇮🇳 हिंदी</option>
      </select>
      <button type="button" onClick={speak} className="rounded-xl border border-slate-200 bg-white/70 px-3 py-1.5 text-xs font-semibold text-slate-800 hover:bg-white dark:border-white/10 dark:bg-white/10 dark:text-slate-100 dark:hover:bg-white/15" title="Speak alert">🔊 Speak</button>
    </div>
  )
}
                  ...
                  <div key={a.id} className="rounded-2xl border border-slate-200/70 bg-white/60 p-4 dark:border-white/10 dark:bg-white/5">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0">
                        ...
                        {a.emergency_response && <EmergencyResponse response={a.emergency_response} />}
